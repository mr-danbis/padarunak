    ...
    <div class="wishlist__container">
      ...
      <template v-else>
        ...
        <template v-if="!showForm">
          ...
          <button type="button" class="wishlist__add-btn" :disabled="loading" @click="openForm">
        ...
        <ul v-else-if="items.length > 0" class="wishlist__list">
          <WishlistItem v-for="item in items" :key="item.id" :item="item" @remove="removeItem(item.id)" />
        </ul>
      </template>
    </div>
  </div>
