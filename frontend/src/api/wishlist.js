...
export async function fetchWishlist() {
  const res = await fetch(`${BASE}/api/wishlist`, CREDENTIALS)
...
export async function deleteWishlistItem(id) {
  const res = await fetch(`${BASE}/api/wishlist/${id}`, { method: 'DELETE', ...CREDENTIALS })
  if (!res.ok && res.status !== 204) throw new Error('Не удалось удалить товар')
}

export async function fetchLinkPreview(url) {
  const encoded = encodeURIComponent(url)
  const res = await fetch(`${BASE}/api/link-preview?url=${encoded}`, CREDENTIALS)
